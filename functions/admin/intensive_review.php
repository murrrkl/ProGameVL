...
<?php
    ...
    echo "<div style='display: flex; justify-content: center;'> <div style=' text-align: left; background-color: white; width: auto; margin-top: 40px; border-radius: 30px; '><table id = 'database'><tr>
        <th> Название </th>
        <th> Описание </th>
        <th> Даты </th>
        <th> Изображение </th>
    ...
    foreach($res as $row){
        ...
        echo "<td class = 'name'><a href='./images/intensive/" . $row["image"] ."'>" . $row["image"] . "</a></td>";
        echo "<td><a class = 'update' href='update_intensive.php?id=" . $row["id"] . "'><button class = 'btn'>Редактировать</button></a></td>";
        echo "<td><form action='./intensive_review.php' method='post'>
    ...
    if(isset($_POST["id"])) {
        $sql = "DELETE FROM intensive WHERE id = :userid";
