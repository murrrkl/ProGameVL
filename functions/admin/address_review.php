...
<?php
    ...
    echo "<div style='display: flex; justify-content: center;'> <div style=' text-align: left; background-color: white; width: auto; margin-top: 40px; border-radius: 30px; '><table id = 'database'><tr><th>Адрес</th><th></th><th></th></tr>";
    foreach($res as $row){
        echo "<tr>";
        echo "<td class = 'name'>" . $row["address"] . "</td>";
        echo "<td><a class = 'update' href='update_address.php?id=" . $row["id"] . "'><button class = 'btn'>Редактировать</button></a></td>";
        echo "<td><form action='./address_review.php' method='post'>
    ...
    if(isset($_POST["id"])) {
        $sql = "DELETE FROM addresses WHERE id = :userid";
